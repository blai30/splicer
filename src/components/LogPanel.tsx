...
import { ScrollText, Trash, X } from 'lucide-preact'
...
export function LogPanel() {
  ...
  // Collapsed: a small flush button in the corner, out of the way.
  if (!logPanelVisible.value) {
    return (
      <div class="flex shrink-0 justify-end">
        <button
          onClick={() => (logPanelVisible.value = true)}
          class="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-medium text-slate-400 transition-colors hover:bg-slate-200/60 hover:text-slate-600 hover:duration-0 dark:text-slate-500 dark:hover:bg-slate-800/60 dark:hover:text-slate-300"
          aria-label="Open logs"
        >
          <ScrollText class="h-3.5 w-3.5" />
          Open logs
          {logs.value.length > 0 && (
            <span class="text-slate-400 dark:text-slate-500">({logs.value.length})</span>
          )}
        </button>
      </div>
    )
    ...
    <div class="flex shrink-0 flex-col gap-3 rounded-lg border border-slate-200/60 bg-slate-50/40 px-4 py-3 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/40">
      <div class="flex w-full items-center justify-between gap-2">
        ...
        <div class="flex items-center gap-2 text-slate-500 dark:text-slate-400">
          ...
          <button
            onClick={() => (logPanelVisible.value = false)}
            class="inline-flex h-7 w-7 items-center justify-center rounded transition-colors hover:bg-slate-100 hover:duration-0 dark:hover:bg-slate-800"
